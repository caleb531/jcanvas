...
( function($) {
		...
		function updatePathGuides(parent) {
			var handles = parent._handles, guides = parent._guides, handle, h, guide, g;
		...
		function addPathGuides($canvas, parent) {
			var handles = parent._handles, prevHandle, nextHandle, otherHandle, handle, h, guide, guideProps;
			guideProps = $.extend({}, parent.guide, {
				layer : true,
				draggable : false,
				type : 'line'
		...
		function updatePathHandles(parent) {
			var handles = parent._handles, handle, h;
			if (handles) {
				// Move handles when dragging
				for ( h = 0; h < handles.length; h += 1) {
					handle = handles[h];
