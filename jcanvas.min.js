...
b.layer.groups={}}return this};g.fn.removeLayerGroup=function(d){var c,a,b,f;if(d!==r)for(a=0;a<this.length;a+=1)if(c=g(this[a]),E(this[a]),c.getLayers(),b=c.getLayerGroup(d))for(b=b.slice(0),f=0;f<b.length;f+=1)c.removeLayer(b[f]);return this};g.fn.addLayerToGroup=function(d,c){var a,b,f,e=[c];for(b=0;b<this.length;b+=1)a=g(this[b]),f=a.getLayer(d),f.groups&&(e=f.groups.slice(0),-1===ha(c,f.groups)&&e.push(c)),a.setLayer(f,{groups:e});return this};g.fn.removeLayerFromGroup=function(d,c){var a,b,
f,e=[],k;for(b=0;b<this.length;b+=1)a=g(this[b]),f=a.getLayer(d),f.groups&&(k=ha(c,f.groups),-1!==k&&(e=f.groups.slice(0),e.splice(k,1),a.setLayer(f,{groups:e})));return this};T.cursors=["grab","grabbing","zoom-in","zoom-out"];T.prefix=function(){var d=getComputedStyle(va.documentElement,"");return"-"+(ta.call(d).join("").match(/-(moz|webkit|ms)-/)||""===d.OLink&&["","o"])[1]+"-"}();g.fn.triggerLayerEvent=function(d,c){var a,b,f;for(b=0;b<this.length;b+=1)a=g(this[b]),f=E(this[b]),(d=a.getLayer(d))&&
O(a,f,d,c);return this};g.fn.drawLayer=function(d){var c,a,b;for(c=0;c<this.length;c+=1)b=g(this[c]),(a=K(this[c]))&&(a=b.getLayer(d))&&a.visible&&a._method&&(a._next=h,a._method.call(b,a));return this};g.fn.drawLayers=function(d){var c,a,b=d||{},f,e,k,r,v,B,z,J;(r=b.index)||(r=0);for(c=0;c<this.length;c+=1)if(d=g(this[c]),a=K(this[c])){v=E(this[c]);b.clear!==w&&d.clearCanvas();a=v.layers;for(k=r;k<a.length;k+=1)if(f=a[k],f.index=k,b.resetFire&&(f._fired=w),B=d,z=f,e=k+1,z&&z.visible&&z._method&&
(z._next=e?e:h,z._method.call(B,z)),f._masks=v.transforms.masks.slice(0),f._method===g.fn.drawImage&&f.visible){J=!0;break}if(J)break;f=v;var y=e=z=B=void 0;B=h;for(z=f.intersecting.length-1;0<=z;z-=1)if(B=f.intersecting[z],B._masks){for(y=B._masks.length-1;0<=y;y-=1)if(e=B._masks[y],!e.intersects){B.intersects=w;break}if(B.intersects&&!B.intangible)break}B&&B.intangible&&(B=h);f=B;B=v.event;z=B.type;if(v.drag.layer){e=d;var y=v,H=z,u=void 0,q=void 0,m=void 0,A=m=void 0,F=void 0,m=u=u=m=void 0,m=
y.drag,A=(q=m.layer)&&q.dragGroups||[],u=y.layers;if("mousemove"===H||"touchmove"===H){if(m.dragging||(m.dragging=p,q.dragging=p,q.bringToFront&&(u.splice(q.index,1),q.index=u.push(q)),q._startX=q.x,q._startY=q.y,q._endX=q._eventX,q._endY=q._eventY,O(e,y,q,"dragstart")),m.dragging)for(u=q._eventX-(q._endX-q._startX),m=q._eventY-(q._endY-q._startY),q.dx=u-q.x,q.dy=m-q.y,q.x=u,q.y=m,O(e,y,q,"drag"),u=0;u<A.length;u+=1)if(m=A[u],F=y.layer.groups[m],q.groups&&F)for(m=0;m<F.length;m+=1)F[m]!==q&&(F[m].x+=
q.dx,F[m].y+=q.dy)}else if("mouseup"===H||"touchend"===H)m.dragging&&(q.dragging=w,m.dragging=w,O(e,y,q,"dragstop")),y.drag={}}e=v.lastIntersected;e===h||f===e||!e._hovered||e._fired||v.drag.dragging||(v.lastIntersected=h,e._fired=p,e._hovered=w,O(d,v,e,"mouseout"),d.css({cursor:v.cursor}));f&&(f[z]||V.mouseEvents[z]&&(z=V.mouseEvents[z]),f._event&&f.intersects&&(v.lastIntersected=f,!(f.mouseover||f.mouseout||f.cursors)||v.drag.dragging||f._hovered||f._fired||(f._fired=p,f._hovered=p,O(d,v,f,"mouseover")),
f._fired||(f._fired=p,B.type=h,O(d,v,f,z)),!f.draggable||f.disableEvents||"mousedown"!==z&&"touchstart"!==z||(v.drag.layer=f)));f!==h||v.drag.dragging||d.css({cursor:v.cursor});k===a.length&&(v.intersecting.length=0,v.transforms=ka(na),v.savedTransforms.length=0)}return this};g.fn.addLayer=function(d){var c,a;for(c=0;c<this.length;c+=1)if(a=K(this[c]))a=new G(d),a.layer=p,N(this[c],a,d);return this};T.props=["width","height","opacity","lineHeight"];T.propsObj={};g.fn.animateLayer=function(){function d(a,
b,c){return function(){var d,f;for(f=0;f<T.props.length;f+=1)d=T.props[f],c[d]=c["_"+d];for(var k in c)c.hasOwnProperty(k)&&-1!==k.indexOf(".")&&delete c[k];b.animating&&b.animated!==c||a.drawLayers();c._animating=w;b.animating=w;b.animated=h;e[4]&&e[4].call(a[0],c);O(a,b,c,"animateend")}}function c(a,b,c){return function(d,f){var k,g,h=!1;"_"===f.prop[0]&&(h=!0,f.prop=f.prop.replace("_",""),c[f.prop]=c["_"+f.prop]);-1!==f.prop.indexOf(".")&&(k=f.prop.split("."),g=k[0],k=k[1],c[g]&&(c[g][k]=f.now));
