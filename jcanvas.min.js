...
function(){var e,b;for(e=0;e<this.length;e+=1)d(this[e]),b=getCanvasData(this[e]),b.layers.length=0,b.layer.names={},b.layer.groups={};return this};d.fn.drawLayer=function b(a){var c,h,g,j;for(c=0;c<this.length;c+=1)g=d(this[c]),h=F(this[c]),j=g.getLayer(a),b(g,h,j);return this};d.fn.drawLayers=function(b){var a,c,h=b||{},g,j,E,J,r,C;h.index||(h.index=0);for(b=0;b<this.length;b+=1)if(a=d(this[b]),c=F(this[b])){E=K(this[b]);for(g=0;g<E.transforms.masks;g+=1)a.restoreCanvas();h.clear!==G&&a.clearCanvas();
c=E.layers;for(j=h.index;j<c.length&&!(g=c[j],g.index=j,h.resetFire&&(g._fired=G),g._event=!g.disableEvents,J=a,r=g,C=j+1,r&&(r.visible&&r._method)&&(r._next=C?C:f,r._method.call(J,r)),g._mousedout&&(g._mousedout=G,g._fired=w,g._hovered=G,g.mouseout&&g.mouseout.call(this[b],g),g.cursor&&g._cursor&&a.css({cursor:g._cursor})),g._method===d.fn.drawImage&&g.visible);j+=1);for(g=0;g<E.transforms.masks;g+=1)a.restoreCanvas();g=E.intersects[E.intersects.length-1]||[];j=E.event;J=j.type;g[J]||ma[J]&&(J=ma[J]);
C=g[J];r=E.drag;if(g._event&&g.withinMask===w){if((g.mouseover||g.mouseout||g.cursor)&&!g._hovered&&!g._fired)g._fired=w,g._hovered=w,g.mouseover&&g.mouseover.call(this[b],g),g.cursor&&(g._cursor=a.css("cursor"),a.css({cursor:g.cursor}));C&&!g._fired&&(g._fired=w,C.call(this[b],g),j.type=f);if(g.draggable&&!g.disableDrag&&("mousedown"===J||"touchstart"===J)){g.bringToFront&&(c.splice(g.index,1),g.index=c.push(g));r.layer=g;r.dragging=w;if(r.layer.group&&r.layer.dragGroupWithLayer&&(a=E.layer.groups[r.layer.group]))for(j=
