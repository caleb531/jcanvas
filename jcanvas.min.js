...
1)*l.height/W.length/2,n.fillText(W[x],s,r),"transparent"!==l.fillStyle&&(n.shadowColor="transparent"),n.strokeText(W[x],s,r);r=0;"top"===l.baseline?r+=l.height/2:"bottom"===l.baseline&&(r-=l.height/2);l._event&&(n.beginPath(),n.fillStyle="rgba(255, 0, 0, 0.5)",n.rect(l.x-l.width/2,l.y-l.height/2+r,l.width,l.height),U(this[v],n,l),n.fill(),n.closePath());l._transformed&&n.restore()}ba.propCache=l;return this};f.fn.measureText=function(g){var f,v;f=this.getLayer(g);if(!f||f&&!f._layer)f=new J(g);if(g=
K(this[0]))Qa(g,f),v=Sa(g,f),Ra(this[0],g,f,v);return f};f.fn.drawImage=function G(v){function n(l,n,s,q,v){return function(){var x=f(l);T(l,n,q);q.width===g&&q.sWidth===g&&(q.width=q.sWidth=I.width);q.height===g&&q.sHeight===g&&(q.height=q.sHeight=I.height);v&&(v.width=q.width,v.height=q.height);q.sWidth!==g&&q.sHeight!==g&&q.sx!==g&&q.sy!==g?(q.width===g&&(q.width=q.sWidth),q.height===g&&(q.height=q.sHeight),q.cropFromCenter||(q.sx+=q.sWidth/2,q.sy+=q.sHeight/2),0>q.sy-q.sHeight/2&&(q.sy=q.sHeight/
2),q.sy+q.sHeight/2>I.height&&(q.sy=I.height-q.sHeight/2),0>q.sx-q.sWidth/2&&(q.sx=q.sWidth/2),q.sx+q.sWidth/2>I.width&&(q.sx=I.width-q.sWidth/2),S(l,n,q,q.width,q.height),n.drawImage(I,q.sx-q.sWidth/2,q.sy-q.sHeight/2,q.sWidth,q.sHeight,q.x-q.width/2,q.y-q.height/2,q.width,q.height)):(S(l,n,q,q.width,q.height),n.drawImage(I,q.x-q.width/2,q.y-q.height/2,q.width,q.height));n.beginPath();n.rect(q.x-q.width/2,q.y-q.height/2,q.width,q.height);U(l,n,q);n.closePath();q._transformed&&n.restore();ya(n,s,
q);q.layer?Q(x,s,v,"load"):q.load&&q.load.call(x[0],v);q.layer&&(v._masks=s.transforms.masks.slice(0),q._next&&x.drawLayers({clear:z,resetFire:w,index:q._next}))}}var l,x,W,s,da,H,I,O,N,R=ba.imageCache;for(x=0;x<this.length;x+=1)if(l=this[x],W=K(this[x]))s=D(this[x]),da=new J(v),H=P(this[x],da,v,G),da.visible&&(N=da.source,O=N.getContext,N.src||O?I=N:N&&(R[N]!==r?I=R[N]:(I=new ua,I.src=N,R[N]=I)),I&&(I.complete||O?n(l,W,s,da,H)():(f(I).bind("load",n(l,W,s,da,H)),I.src=I.src)));return this};f.fn.createPattern=
function(G){function v(){s=l.createPattern(r,x.repeat);x.load&&x.load.call(n[0],s)}var n=this,l,x,r,s,w;(l=K(n[0]))?(x=new J(G),w=x.source,ca(w)?(r=f("<canvas />")[0],r.width=x.width,r.height=x.height,G=K(r),w.call(r,G),v()):(G=w.getContext,w.src||G?r=w:(r=new ua,r.src=w),r.complete||G?v():(f(r).bind("load",v),r.src=r.src))):s=g;return s};f.fn.createGradient=function(f){var v,n=[],l,x,w,s,z,D,I;f=new J(f);if(v=K(this[0])){f.x1=f.x1||0;f.y1=f.y1||0;f.x2=f.x2||0;f.y2=f.y2||0;v=f.r1!==g&&f.r2!==g?v.createRadialGradient(f.x1,
f.y1,f.r1,f.x2,f.y2,f.r2):v.createLinearGradient(f.x1,f.y1,f.x2,f.y2);for(s=1;f["c"+s]!==r;s+=1)f["s"+s]!==r?n.push(f["s"+s]):n.push(g);l=n.length;n[0]===g&&(n[0]=0);n[l-1]===g&&(n[l-1]=1);for(s=0;s<l;s+=1){if(n[s]!==g){D=1;I=0;x=n[s];for(z=s+1;z<l;z+=1)if(n[z]!==g){w=n[z];break}else D+=1;x>w&&(n[z]=n[s])}else n[s]===g&&(I+=1,n[s]=x+(w-x)/D*I);v.addColorStop(n[s],f["c"+(s+1)])}}else v=g;return v};f.fn.setPixels=function v(f){var l,x,r,s,w,z,I,D,H;for(x=0;x<this.length;x+=1)if(l=this[x],r=K(l)){s=
new J(f);P(l,s,f,v);S(this[x],r,s,s.width,s.height);if(s.width===g||s.height===g)s.width=l.width,s.height=l.height,s.x=s.width/2,s.y=s.height/2;if(0!==s.width&&0!==s.height){z=r.getImageData(s.x-s.width/2,s.y-s.height/2,s.width,s.height);I=z.data;H=I.length;if(s.each)for(D=0;D<H;D+=4)w={r:I[D],g:I[D+1],b:I[D+2],a:I[D+3]},s.each.call(l,w,s),I[D]=w.r,I[D+1]=w.g,I[D+2]=w.b,I[D+3]=w.a;r.putImageData(z,s.x-s.width/2,s.y-s.height/2);r.restore()}}return this};f.fn.getCanvasImage=function(f,n){var l,x=g;
0!==this.length&&(l=this[0],l.toDataURL&&(n===r&&(n=1),x=l.toDataURL("image/"+f,n)));return x};f.fn.detectPixelRatio=function(g){var n,l,r,z,s,J,H;for(l=0;l<this.length;l+=1)n=this[l],f(this[l]),r=K(n),H=D(this[l]),H.scaled||(z=window.devicePixelRatio||1,s=r.webkitBackingStorePixelRatio||r.mozBackingStorePixelRatio||r.msBackingStorePixelRatio||r.oBackingStorePixelRatio||r.backingStorePixelRatio||1,z/=s,1!==z&&(s=n.width,J=n.height,n.width=s*z,n.height=J*z,n.style.width=s+"px",n.style.height=J+"px",
r.scale(z,z)),H.pixelRatio=z,H.scaled=w,g&&g.call(n,z));return this};O.clearCache=function(){for(var f in ba)ba.hasOwnProperty(f)&&(ba[f]={})};f.support.canvas=f("<canvas />")[0].getContext!==r;O.defaults=ma;O.prefs=$;O.setGlobalProps=T;O.transformShape=S;O.detectEvents=U;O.closePath=X;f.jCanvas=O})(jQuery,document,Image,Array,Math,parseFloat,console,!0,!1,null);
